...
def main():
    ...
    if hasattr(cfg.dataset, "hr_fps_factor"):
        USE_HR_LR = True
        train_paths = train_paths[::cfg.dataset.hr_fps_factor]
        train_paths_lr = glob.glob(os.path.join(cfg.dataset.cachedir_lr, "train", "*.data"))
    else:
        USE_HR_LR = False
    ...
    for i in trange(start_iter, cfg.experiment.train_iters):
        ...
        if USE_HR_LR and i % cfg.dataset.hr_bias != 0:
            datafile = np.random.choice(train_paths_lr)
        else:
